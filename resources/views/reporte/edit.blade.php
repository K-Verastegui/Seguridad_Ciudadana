            ...
            @foreach ($reportes as $reporte)
                <form action="{{route('reporte.update',$reporte->id)}}" method="post">
                    ...
                    <div style="width: 47%; float: left;">
                        <input type="hidden" class="form-control" value="{{ $incidencia_id }}" name="incidencia_id">
                    <br>
                        <label for="description">Descripcion detallada<span class="required">*</span></label>
                        <textarea type="text" id="description" class="form-control" name="description" >{{$reporte->description}}</textarea><br>
                        
                        <label for="objetos">Objetos abandonadas por los delincuentes<span class="required">*</span></label>
                        <textarea type="text" id="objetos" class="form-control" name="objetos" >{{$reporte->objetos}}</textarea><br>
                    </div>
                    <div style="width: 47%; float: right;">
                    <br>
                        <label for="aprhendido">Aprhendido(s) <span class="required">*</span></label>
                        <textarea type="aprhendido" id="aprhendido" class="form-control" name="aprhendido">{{$reporte->aprhendido}}</textarea><br>
                    
                        <label for="seguido">Estado <span class="required">*</span></label>
                        <select name="seguido" id="seguido" class="form-control" required>
